...
use App\Controllers\{MarcaController, CarroController};
    ...
    public static function run($basepath = '', $case_matters = false, $trailing_slash_matters = false, $multimatch = false)
    {
        ...
        foreach (self::$routes as $route) {
                ...
                foreach ((array)$route['method'] as $allowedMethod) {
                    ...
                    if (strtolower($method) == strtolower($allowedMethod)) {
                        ...
                        if ( $return_value = call_user_func_array($route['function'], $matches) ) {
                            list($controller, $action) = explode('@', $return_value);
                        }
            ...
            if ($route_match_found && !$multimatch) {
                $refl = new \ReflectionClass('App\Controllers\\'. $controller);
                $instance = $refl->newInstance();

                // Verifica se existe um segundo parâmetro
                $param = explode('/', ltrim($path, '/') );
                array_shift($param);
                ...
                echo count($param) > 0 ? $instance->{$action}($param[0]) : $instance->{$action}();

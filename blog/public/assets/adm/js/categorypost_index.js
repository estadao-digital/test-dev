$(function () {
    load_table();
    action_form("#bulkaction-categorypost");
...
function load_table() {
    var $table = $("table#table-categorypost"),
        identifier = {
            reorder: "reorder-table-categorypost",
            colvis: "colvis-table-categorypost",
            colwidth: "colwidth-table-categorypost"
        };
...
function delete_categorypost(id) {
    var data = {id: id};
    $.post('ws/categorypost/delete/' + id, data, function (data) {
        if (!data.error) {
            localMsg(data);

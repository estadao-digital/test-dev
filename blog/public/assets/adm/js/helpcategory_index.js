$(function () {
    load_table();
    action_form("#bulkaction-helpcategory");
...
function load_table() {
    var $table = $("table#table-helpcategory"),
        identifier = {
            reorder: "reorder-table-helpcategory",
            colvis: "colvis-table-helpcategory",
            colwidth: "colwidth-table-helpcategory"
        };
...
function delete_helpcategory(id) {
    var data = {id: id};
    $.post('ws/helpcategory/delete/' + id, data, function (data) {
        if (!data.error) {
            localMsg(data);

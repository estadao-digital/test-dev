...
function load_table() {
    var $table = $("table#table-level"),
        identifier = {
            reorder: "reorder-table-level",
            colvis: "colvis-table-level",
            colwidth: "colwidth-table-level"
        };
...
function delete_store(id) {
    $('.now_loading').show();
    var data = {id: id};
    $.post('ws/level/delete/' + id, data, function (data) {
        if (!data.error) {
            localMsg(data);

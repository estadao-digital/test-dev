...
namespace KrupaBOX\Internal\Library\Plain\GhediPunk
{
    class WebSocketUser
    {
        public $socket;
    ...
    abstract class WebSocketServer
    {
        ...
        protected $userClass = 'KrupaBOX\Internal\Library\Plain\GhediPunk\WebSocketUser';
            ...
            foreach ($this->heldMessages as $key => $hm) {
                ...
                foreach ($this->users as $currentUser) {
                    if ($hm['user']->socket == $currentUser->socket) {
                        $found = true;
                        if ($currentUser->handshake) { unset($this->heldMessages[$key]); $this->send($currentUser, $hm['message']);  }
                    }
            ...
            while (true)
            {
                ...
                foreach ($read as $socket) {
                    if ($socket == $this->master) {
                        ...
                        } else { $this->connect($client); $this->stdout("Client connected. " . $client); }
                    } else {
                        ...
                        if ($numBytes === false) {
                            ...
                            switch ($sockErrNo) {
                                ...
                                case 125: // ECANCELED    -- Operation canceled
                                    ...
                                    break;
                                default: $this->stderr('Socket error: ' . socket_strerror($sockErrNo));
                            }
                        } elseif ($numBytes == 0) { $this->disconnect($socket); $this->stderr("Client disconnected. TCP connection lost: " . $socket);
                        } else {
                            ...
                            } else { $this->split_packet($numBytes, $buffer, $user);  }
                        }
                    }
                }
            }
        ...
        protected function disconnect($socket, $triggerClosed = true, $sockErrNo = null)
        {
            ...
            if ($disconnectedUser !== null) {
                ...
                if (array_key_exists($disconnectedUser->id, $this->sockets)) { unset($this->sockets[$disconnectedUser->id]); }
                if (!is_null($sockErrNo)) { socket_clear_error($socket); }
                if ($triggerClosed) {
        ...
        protected function doHandshake($user, $buffer)
        {
            ...
            if (isset($headers['get'])) { $user->requestedResource = $headers['get']; }
            else { $handshakeResponse = "HTTP/1.1 405 Method Not Allowed\r\n\r\n"; }

            if (!isset($headers['host']) || !$this->checkHost($headers['host'])) { $handshakeResponse = "HTTP/1.1 400 Bad Request"; }
            if (!isset($headers['upgrade']) || strtolower($headers['upgrade']) != 'websocket') { $handshakeResponse = "HTTP/1.1 400 Bad Request"; }
            if (!isset($headers['connection']) || strpos(strtolower($headers['connection']), 'upgrade') === FALSE) { $handshakeResponse = "HTTP/1.1 400 Bad Request"; }

            if (!isset($headers['sec-websocket-key'])) { $handshakeResponse = "HTTP/1.1 400 Bad Request"; }
            else if (!isset($headers['sec-websocket-version']) || strtolower($headers['sec-websocket-version']) != 13) { $handshakeResponse = "HTTP/1.1 426 Upgrade Required\r\nSec-WebSocketVersion: 13"; }

            if (($this->headerOriginRequired && !isset($headers['origin'])) || ($this->headerOriginRequired && !$this->checkOrigin($headers['origin']))) { $handshakeResponse = "HTTP/1.1 403 Forbidden"; }
            if (($this->headerSecWebSocketProtocolRequired && !isset($headers['sec-websocket-protocol'])) || ($this->headerSecWebSocketProtocolRequired && !$this->checkWebsocProtocol($headers['sec-websocket-protocol']))) { $handshakeResponse = "HTTP/1.1 400 Bad Request"; }
            if (($this->headerSecWebSocketExtensionsRequired && !isset($headers['sec-websocket-extensions'])) || ($this->headerSecWebSocketExtensionsRequired && !$this->checkWebsocExtensions($headers['sec-websocket-extensions']))) { $handshakeResponse = "HTTP/1.1 400 Bad Request"; }
            ...
            $rawToken = "";
            for ($i = 0; $i < 20; $i++) { $rawToken .= chr(hexdec(substr($webSocketKeyHash, $i * 2, 2))); }
        ...
        protected function checkOrigin($origin) { return true; }
        protected function checkWebsocProtocol($protocol) { return true; }
        protected function checkWebsocExtensions($extensions) { return true; }
        protected function processProtocol($protocol) { return ""; }
        protected function processExtensions($extensions)  { return ""; }
        ...
        protected function stdout($message) { return null; /* if ($this->interactive) { echo "$message\n"; }*/ }
        protected function stderr($message) { return null; /* if ($this->interactive) { echo "$message\n"; } */ }
        ...
        {
            $offset = 2;
            if ($headers['hasmask']) { $offset += 4;  }
            if ($headers['length'] > 65535) { $offset += 8; }
            elseif ($headers['length'] > 125) {  $offset += 2; }
            return $offset;
        }
        ...
        protected function deframe($message, &$user)
        {
            ...
            if ($this->checkRSVBits($headers, $user)) { return false;  }
            if ($willClose) { return false;  }
        ...
        {
            $effectiveMask = "";
            if ($headers['hasmask']) {  $mask = $headers['mask']; }
            else { return $payload;  }
        ...
        {
            if (ord($headers['rsv1']) + ord($headers['rsv2']) + ord($headers['rsv3']) > 0) { return true; }
            return false;
        }
...
namespace WebSocket
{
    class Server extends \KrupaBOX\Internal\Library\Plain\GhediPunk\WebSocketServer
    {
        protected $host = null;
        protected $port = null;
        ...
        public function sendMessageByConnectionId($connectionId, $message)
        {
            ...
            if ($this->connections->containsKey($connectionId)) {
                ...
                if ($connection->tunnel->socket != null)
                    $this->send($connection->tunnel, $message);
            }
        }
        ...
        public function closeByConnectionId($connectionId)
        {
            ...
            if ($this->connections->containsKey($connectionId)) {
                $connection = $this->connections[$connectionId];
                if ($connection->tunnel->socket != null)
                    $this->disconnect($connection->tunnel->socket);
            }
        }

        protected function process($user, $message) // Hook
        {
            $connection = null;
            foreach ($this->connections as $_connection)
                if ($_connection->tunnel->id == $user->id)
            ...
            if (\Connection::isCommandLineRequest())
                echo ("[tcp://" . ($this->host . ":" . $this->port) . "] [#" . $connectIdPad . "] <= [Message] " . $message . "\n");
        ...
        protected function connected($user)  // Hook
        {
            $this->lastConnectionId++;

            $connection = new Server\Connection($this, $user, $this->lastConnectionId);
            $this->connections[$this->lastConnectionId] = $connection;

            $connectIdPad = intEx($connection->connectionId)->toPad(\intEx::PAD_LEFT, 10);
            if (\Connection::isCommandLineRequest())
                echo ("[tcp://" . ($this->host . ":" . $this->port) . "] [#" . $connectIdPad . "] <= [Connection] " . $connection->remoteAddress . "\n");
        ...
        protected function closed($user)  // Hook
        {
            $connection = null;
            foreach ($this->connections as $_connection)
                if ($_connection->tunnel->id == $user->id)
            ...
            $connectIdPad = intEx($connection->connectionId)->toPad(\intEx::PAD_LEFT, 10);
            if (\Connection::isCommandLineRequest())
                echo ("[tcp://" . ($this->host . ":" . $this->port) . "] [#" . $connectIdPad . "] <= [Close]\n");

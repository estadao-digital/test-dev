...
namespace Cache
{
    class Render
    {
        ...
        public static function __onInitialize()
        {
            ...
            $config = \Config::get();
            if ($config->render->cache == true)
            {
                $type = $config->render->cacheType;
                ...
                if (self::$cacheType == memcache)
                {
                    ...
                    self::$cacheServer->connect($config->render->cacheHost, $config->render->cachePort);
                }
                elseif (self::$cacheType == memcached)
                {
                    self::$cacheServer = new \Memcached();
                    self::$cacheServer->addServer($config->render->cacheHost, $config->render->cachePort);
                }

...
namespace Cache
{
    class Database
    {
        ...
        public static function __onInitialize()
        {
            ...
            {
                $type = $config->database->cacheType;
                ...
                if (self::$cacheType == memcache)
                {
                    ...
                    self::$cacheServer->connect($config->database->cacheHost, $config->database->cachePort);
                }
                elseif (self::$cacheType == memcached)
                {
                    self::$cacheServer = new \Memcached();
                    self::$cacheServer->addServer($config->database->cacheHost, $config->database->cachePort);
                }

...
class itrAcmeClient {
  ...
  public function init() {
    ...
    if ($this->initDone) {
      ...
      return Arr([error => OBJECT_ALREADY_INITIALIZED]);
//      throw new \RuntimeException('Object already initialised!', 500);
    ...
    if (is_array($this->certAccountContact) && (in_array('mailto:cert-admin@example.com', $this->certAccountContact) || in_array('tel:+12025551212', $this->certAccountContact))) {
      $this->log('Contact information has not been changed!', 'exception');
        return Arr([error => INTERNAL_SERVER_ERROR, message => 'Contact information has not been changed!']);
//      throw new \RuntimeException('Contact information has not been changed!', 400);
    ...
    if (!is_array($this->directory) ||
      ...
      $this->log('Directory information are incomplete!', 'exception');
        return Arr([error => INTERNAL_SERVER_ERROR, message => 'Directory information are incomplete!']);
    ...
    if (is_file($this->certAccountDir . '/' . $this->getKeyPrefix($keyType) . 'private.key')) {
      ...
      if ($this->lastResponse['status'] !== 200) {
        $this->log('Account validation failed: ' . $this->lastResponse['status'], 'exception');
          return Arr([error => INTERNAL_SERVER_ERROR, message => 'Account validation failed: ' . $this->lastResponse['status']]);
//        throw new \RuntimeException('Account validation failed: ' . $this->lastResponse['status'], 500);
      ...
      if (preg_match('/Location: (.+)/i', $this->lastResponse['header'], $matches)) {
        ...
        $this->log('Could not get key identifier!', 'exception');
          return Arr([error => INTERNAL_SERVER_ERROR, message => 'Could not get key identifier!']);
    ...
    if ($this->lastResponse['status'] !== 201) {
      $this->log('Account registration failed: ' . $this->lastResponse['status'], 'exception');
        return Arr([error => INTERNAL_SERVER_ERROR, message => 'Account registration failed: ' . $this->lastResponse['status']]);
  ...
  public function signDomains(array $domains) {
    ...
    if ($privateAccountKey === false) {
      $this->log('Cannot read private account key: ' . openssl_error_string(), 'exception');
        return Arr([error => INTERNAL_SERVER_ERROR, message => 'Cannot read private account key: ' . openssl_error_string()]);
      ...
      foreach ($domains as $domain) {
        ...
        try {
          ...
          if (!$this->challengeManager->validateDomainControl($localDomain)) {
              return Arr([error => INTERNAL_SERVER_ERROR, message => 'Failed to validate control of ' . $domain]);
            throw new \RuntimeException('Failed to validate control of ' . $domain, 500);
          }
        } catch (\RuntimeException $e) {
          $this->log($e->getMessage(), 'exception');
            return Arr([error => INTERNAL_SERVER_ERROR, message => $e->getMessage() . $domain]);
          throw $e;
        }
      }
   ...
   */
  public function getSct($certificate, $chain) {
   ...
   */
  protected function generateKey($outputDir = false, $keyType = 'RSA') {
   ...
   * @return string The Diffie-Hellman Parameters as pem
   */
  public function getEcParameters($curve = 'prime256v1') {
...
//      $path .= '/.well-known/acme-challenge';
    ...
    $path = \File\Wrapper::parsePath("cache://.certificate/challenge");
    return $path;
  }
   ...
   */
  protected function getKeyPrefix($keyType) {
...
interface itrAcmeChallengeManager {
   ...
   */
  public function prepareChallenge($domain, array $challenge, array $accountKeyDetails);
   ...
   */
  public function deployChallenge($fqdn, $signedToken, $token);
   ...
   */
  public function getDnsInformation($fqdn);
...
//    chmod($domainWellKnownPath, $this->itrAcmeClient->webServerFilePerm);
  ...
  public function deployChallenge($fqdn, $signedToken, $token) {
    ...
    // Set webserver compatible permissions
      @chmod($domainWellKnownPath . '/' . $token, $this->itrAcmeClient->webServerFilePerm);
...
//    }
   ...
   * @return void
   */
  public function cleanupChallenge($domain, array $challenge) {
    // get the well-known path, we know that it already exists and we can write to it
    $domainWellKnownPath = $this->itrAcmeClient->getDomainWellKnownPath($domain);
...
class RestHelper {
   ...
   */
  public static function get($url, array $obj = [], $return = 'complete') {
   ...
   * @return array|string  The result
   */
  public static function head($url, array $obj = [], $return = 'complete') {
   ...
   * @return array|string  The result
   */
  public static function post($url, $obj = [], $return = 'complete') {
   ...
   * @return array|string  The result
   */
  public static function put($url, array $obj = [], $return = 'complete') {
   ...
   * @return array|string  The result
   */
  public static function delete($url, array $obj = [], $return = 'complete') {
   ...
   */
  public static function execCurl($curl, $return = 'complete') {
